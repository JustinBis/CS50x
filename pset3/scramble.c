 ...
 *
 * Additional code by Justin Bisignano
 * CS50x Fall 2012
 ***************************************************************************/
...
void draw(void)
{
    // leave a clear line at the top
    printf("\n");
    
    for (int i = 0; i<DIMENSION; i++)
    {
        // indent the grid for easy reading
        printf("\t");
        
        for (int j=0; j<DIMENSION; j++)
            printf("%c ", grid[i][j]);
        
        // remeber to start a new line after printing each row    
        printf("\n");
    }
    
    // and end with an extra new line for cleanliness
    printf("\n");
}
...
bool lookup(string s)
{
    ...
    // run until there are no more elements to split. This will happen when max becomes lower than min
    while(max>=min)
    {        
        // calculates the middle index of the array we are searching, offset by the minimum index value of the array
        mid = ((max-min)/2) + min;
        
        // compare strings
        cmp = strcmp(s, dictionary.words[mid].letters);

        // if s is before the middle word, redo the search with just the lower half
        if (cmp<0)
            max = mid - 1;
            
        // if s is before the middle word, redo the search with just the upper half
        else if (cmp>0)
            min = mid + 1;
            
        // if s matches the middle word
        else if (cmp==0){
            // and hasn't already been found, flag it as found and return true
            if (!dictionary.words[mid].found)
            {
                dictionary.words[mid].found = true;
                return true;
            }
            // if it has been found before, return false
            else
                return false;
        }
    }
    
    // since the search failed, return false
    return false;
    
    
    /* PREVIOUS IMPLEMENTATION*********
    
    // performs a linear search to match the word
    // could be improved by seperating by first character "s[0] == ?" and an indexing switch. Or creatively use STRCMP's return
    // I'll do that soon, but for now, this is submittable.
    for (int i=0; i<WORDS; i++)
    { iter++;
        // If the word is matched to a word in the dictionary
        if (strcmp(s, dictionary.words[i].letters) == 0)
            // and if the word hasn't already been found
            if (!dictionary.words[i].found)
            {
                // flag it as found and return true
...
void scramble(void)
{
    // define a temporary grid and copy the original grid to it
    char tmp[DIMENSION][DIMENSION];
    for (int i = 0; i<DIMENSION; i++)
        for (int j=0; j<DIMENSION; j++)
            tmp[i][j] = grid[i][j];
    ...
    for (int i = 0; i<DIMENSION; i++)
        for (int j=0; j<DIMENSION; j++)
            grid[j][DIMENSION-1-i] = tmp[i][j];
}

/**
 * Changes the characters in string s to uppercase characters
 */
void makeupper(string s)
{
    int i = 0;
